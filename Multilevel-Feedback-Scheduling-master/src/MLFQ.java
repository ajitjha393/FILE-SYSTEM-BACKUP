	...
	*/
	...
	public void actionPerformed(ActionEvent ev){
		...
		while(!arrival_queue.isEmpty() && arrival_queue.peek().arrival_time == current_time){
			...
			//MLFQFrame.processesLabel[0][0].setText(buffer.name);
...
//					MLFQ.queueLabels.get(x).add(label);
//				}else{
...
//	}
	...
	private void update_GANTT_CHART() {
		...
		MLFQFrame.GANTT_TIME.add(label = new JLabel(String.valueOf(current_time)));
		MLFQFrame.timePanel.setLayout(new GridLayout(1,MLFQFrame.GANTT_TIME.size()));

		for(JLabel timeLabel : MLFQFrame.GANTT_TIME) {
			MLFQFrame.timePanel.add(timeLabel);
			MLFQFrame.timePanel.revalidate();
		}

		// gantt chart gcPanel
		MLFQFrame.GANTT_CHART_LABELS.add(label = new JLabel(in_CPU != null ? in_CPU.name : "  ", javax.swing.SwingConstants.CENTER));
		label.setOpaque(true);
		label.setForeground(java.awt.Color.WHITE);
		label.setFont(new java.awt.Font("Calibri", java.awt.Font.BOLD, 20));
		label.setBackground(in_CPU != null ? in_CPU.color : Color.WHITE);

		MLFQFrame.gcPanel.setLayout(new GridLayout(1,MLFQFrame.GANTT_CHART_LABELS.size()));
		for(JLabel gc : MLFQFrame.GANTT_CHART_LABELS) {
			MLFQFrame.gcPanel.add(gc);
			MLFQFrame.gcPanel.revalidate();
		}

		MLFQFrame.bottomPanel[0].add(MLFQFrame.timePanel, BorderLayout.NORTH);
		MLFQFrame.bottomPanel[0].add(MLFQFrame.gcPanel, BorderLayout.CENTER);
		MLFQFrame.bottomPanel[0].revalidate();
	}
