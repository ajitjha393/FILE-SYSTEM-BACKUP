public class ProcessQueue{
	...
	public void toFrame(int queueNo){
		...
		PCB.PCBLink buff = first;
		try{
			for(int x = 0; x < MLFQFrame.numOfProcesses; x++) {
				MLFQFrame.processesLabel[queueNo][x].setBackground(java.awt.Color.WHITE);
				MLFQFrame.processesLabel[queueNo][x].setText(null);
			}
			while(!(buff.value == null)){
				//ret += buff.value.name +"|";
				MLFQFrame.processesLabel[queueNo][i].setBackground(buff.value.color);
				MLFQFrame.processesLabel[queueNo][i].setText(buff.value.name);
